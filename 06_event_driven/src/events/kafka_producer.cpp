...
bool KafkaProducer::publish(const Event& event) {
    std::string key = extractPatientId(event);
    auto payload = event.toJson();
...
std::string KafkaProducer::extractPatientId(const Event& event) {
    Poco::JSON::Parser parser;
    auto parsed = parser.parse(event.toJson()).extract<Poco::JSON::Object::Ptr>();
    auto payload = parsed->getObject("payload");
    
    if (payload && payload->has("patient_id")) {
        return payload->getValue<std::string>("patient_id");
    }
