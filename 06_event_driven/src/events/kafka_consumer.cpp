...
public:
    explicit EventHandlerCb(KafkaConsumer* consumer) : consumer_(consumer) {}
    ...
    , running_(false)
{
    ...
    eventHandler_.reset(new EventHandlerCb(this));
...
void KafkaConsumer::processMessage(RdKafka::Message* msg) {
    ...
    try {
        ...
        if (eventType == "MedicalRecordCreated") {
            handleMedicalRecordCreated(eventJson);
        } else if (eventType == "MedicalRecordUpdated") {
            handleMedicalRecordUpdated(eventJson);
        } else if (eventType == "MedicalRecordStatusChanged") {
            handleMedicalRecordStatusChanged(eventJson);
        }
...
void KafkaConsumer::markProcessed(const std::string& eventId) {
    ...
    if (processedEvents_.size() > 10000) {
        ...
        std::advance(it, processedEvents_.size() / 2);
        processedEvents_.erase(processedEvents_.begin(), it);
    }
}

void KafkaConsumer::handleMedicalRecordCreated(Poco::JSON::Object::Ptr eventJson) {
    auto payload = eventJson->getObject("payload");
    std::string recordId = payload->getValue<std::string>("record_id");
    std::string patientId = payload->getValue<std::string>("patient_id");
    Poco::Logger::get("KafkaConsumer").information(
        Poco::format("Processed MedicalRecordCreated: %s (patient: %s)", 
                    recordId, patientId));
}

void KafkaConsumer::handleMedicalRecordUpdated(Poco::JSON::Object::Ptr) {}
void KafkaConsumer::handleMedicalRecordStatusChanged(Poco::JSON::Object::Ptr) {}
